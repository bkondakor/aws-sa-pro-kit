# Advanced Scenario Questions - Batch 2
## Domain 2: Design for New Solutions (15 Questions)

### Question 1: Lambda Cold Start with VPC and Provisioned Concurrency
**Scenario:** A financial trading application uses Lambda functions connected to a VPC to access RDS databases. Even with provisioned concurrency configured for 50 concurrent executions, users experience occasional 10-15 second delays during peak hours. CloudWatch metrics show that provisioned concurrency is never exhausted. What is the MOST likely cause?

**Options:**
A) Provisioned concurrency doesn't eliminate cold starts for VPC-connected Lambda functions
B) The Lambda function's memory allocation is too low, causing slow initialization
C) Application Auto Scaling for provisioned concurrency is reacting too slowly to traffic spikes
D) Bursts beyond provisioned concurrency still experience ENI creation delays for VPC connections
...
### Question 2: ECS Task Placement with Binpack and Spread Strategies
**Scenario:** An architect designs an ECS cluster with 10 EC2 instances for a microservices application. They use the binpack placement strategy to maximize resource utilization. During a rolling deployment, several tasks fail to place despite sufficient aggregate cluster resources. What is the PRIMARY issue?

**Options:**
A) Binpack strategy can create fragmentation where no single instance has enough resources for new task placement
B) ECS doesn't support rolling deployments with binpack strategy; only spread strategy supports it
C) The cluster instances need to be in an Auto Scaling group to support binpack placement
D) Binpack strategy has a hard limit of 5 tasks per instance regardless of available resources
...
### Question 3: Step Functions Express vs Standard Workflow Choice
**Scenario:** A company processes millions of IoT sensor events per day. Each event requires a simple 3-step transformation workflow taking 30 seconds. They need exactly-once processing semantics and must maintain an audit trail of all executions for 90 days. Which Step Functions workflow type should they use?

**Options:**
A) Express workflows in synchronous mode for exactly-once semantics with CloudWatch Logs for audit trail
B) Standard workflows because they provide exactly-once execution and built-in 90-day execution history
C) Express workflows in asynchronous mode with DynamoDB for tracking execution state
D) Standard workflows with cross-region replication for durability and compliance
...
### Question 4: DynamoDB GSI with Write Throttling
**Scenario:** A DynamoDB table uses on-demand capacity mode. The table has a Global Secondary Index (GSI) with a different partition key. During traffic spikes, write operations to the base table are being throttled even though the table is in on-demand mode. What is the MOST likely cause?

**Options:**
A) On-demand mode has a per-partition throughput limit that's being exceeded during spikes
B) The GSI is throttling writes because GSI projections are being written faster than DynamoDB can handle
C) DynamoDB automatically switches to provisioned mode during extreme spikes, causing throttling
D) On-demand mode requires time to scale up, and instant traffic spikes exceed the adaptive scaling rate
...
### Question 5: Aurora Global Database Write Forwarding Limitations
**Scenario:** A global application uses Aurora Global Database with a primary cluster in us-east-1 and secondary clusters in eu-west-1 and ap-southeast-1. To reduce latency for European users writing data, they enable write forwarding on the eu-west-1 secondary cluster. After enabling it, they notice write performance for European users is worse than before. Why?

**Options:**
A) Write forwarding requires synchronous replication to the primary, increasing latency compared to direct writes to primary
B) Write forwarding only works for INSERT operations, not UPDATE or DELETE, causing application errors
C) The secondary cluster's instance class must match the primary for write forwarding to work efficiently
D) Write forwarding is incompatible with Aurora Serverless v2 instances in the secondary cluster
...
### Question 6: EKS Pod Identity vs IRSA
**Scenario:** An architect is configuring AWS service access for EKS pods. They need pods to access S3 and DynamoDB with different IAM permissions per pod. They're choosing between IRSA (IAM Roles for Service Accounts) and EKS Pod Identity. The cluster has 500 pods with 50 different service accounts requiring different permissions. What should they consider?

**Options:**
A) IRSA has a limit of 100 service account to IAM role mappings per cluster, requiring EKS Pod Identity instead
B) EKS Pod Identity provides better performance with no webhook latency, making it preferable for large-scale deployments
C) IRSA requires manual OIDC provider setup per cluster while Pod Identity is automatic, reducing operational overhead
D) Both solutions work at scale; Pod Identity is newer with simpler configuration but IRSA is more widely supported across Kubernetes distributions
...
### Question 7: API Gateway Regional vs Edge-Optimized
**Scenario:** A company has an API hosted on API Gateway serving customers globally. They use CloudFront with custom domain names and WAF for security. The API is configured as edge-optimized. Their security team wants to use API Gateway resource policies to restrict access to specific VPC endpoints. What issue will they encounter?

**Options:**
A) Edge-optimized APIs don't support resource policies; they must switch to Regional API endpoints
B) Resource policies work with edge-optimized APIs but VPC endpoint restrictions only work with Regional APIs
C) API Gateway resource policies cannot be used together with CloudFront distributions
D) VPC endpoint restrictions require PrivateLink, which is incompatible with edge-optimized APIs
...
### Question 8: EventBridge Cross-Account Event Bus Permissions
**Scenario:** Organization A needs to send events from their account to Organization B's EventBridge event bus. Organization B creates an event bus policy allowing Organization A's account to PutEvents. After testing, events from Organization A are not appearing in Organization B's bus. The PutEvents calls succeed with no errors. What is missing?

**Options:**
A) EventBridge requires resource-based policies on both the source and target event buses for cross-account delivery
B) Organization A must create an EventBridge rule that targets Organization B's event bus ARN
C) Cross-account event delivery requires AWS Organizations trust relationship between the accounts
D) Organization A's IAM principals need explicit permissions to PutEvents on the remote event bus in addition to the resource policy
...
### Question 9: AppSync Resolver Caching Behavior
**Scenario:** A mobile application uses AWS AppSync with DynamoDB as the data source. To improve performance, they enable caching at the API level with a 1-hour TTL. Users report seeing stale data even after making mutations that should update the data. What is the MOST appropriate solution?

**Options:**
A) Reduce the cache TTL to 5 minutes to ensure fresher data
B) Implement cache invalidation logic in mutation resolvers to clear affected cached queries
C) Disable caching for mutations while keeping it enabled for queries
D) Use per-resolver caching instead of API-level caching for better control
...
### Question 10: RDS Proxy Connection Pooling with Lambda
**Scenario:** A serverless application with thousands of Lambda functions connects to an RDS PostgreSQL database through RDS Proxy for connection pooling. During load testing, they still encounter "too many connections" errors from PostgreSQL despite Proxy being configured for 1000 max connections. What is the issue?

**Options:**
A) RDS Proxy doesn't support PostgreSQL, only MySQL and Aurora
B) The RDS instance's max_connections parameter is set lower than the Proxy's max connections setting
C) Lambda functions are not reusing the RDS Proxy connections properly due to incorrect initialization
D) RDS Proxy requires all Lambda functions to be in the same VPC subnet for connection sharing
...
### Question 11: CloudFront Signed URLs vs Signed Cookies
**Scenario:** A video streaming platform needs to provide authenticated access to premium content stored in S3. Users should access multiple video files per session without requesting new authorization. The platform has a mix of web and mobile applications. Which approach is MOST appropriate?

**Options:**
A) CloudFront signed URLs because they provide better security with individual file authorization
B) CloudFront signed cookies because they allow access to multiple files with a single authentication
C) CloudFront signed URLs because signed cookies don't work with mobile applications
D) CloudFront signed cookies because they don't expose the signature in the URL, improving security
...
### Question 12: Kinesis Data Streams Enhanced Fan-Out Limitations
**Scenario:** A data processing application consumes from a Kinesis Data Stream with 50 shards. They have 8 Lambda functions processing the stream, each needing dedicated throughput. They enable enhanced fan-out for all consumers. After deployment, some consumers experience throttling. What is the MOST likely cause?

**Options:**
A) Enhanced fan-out has a limit of 5 registered consumers per stream
B) Lambda functions cannot use enhanced fan-out; only Kinesis Client Library (KCL) applications can
C) The account has reached the enhanced fan-out data transfer quota (2 MB/s per shard per consumer)
D) Enhanced fan-out requires one shard per consumer, and 50 shards can only support 5 consumers with proper distribution
...
### Question 13: ECS Service Discovery with Private DNS Namespace
**Scenario:** An ECS application uses AWS Cloud Map for service discovery with a private DNS namespace. Services in VPC-A can resolve and connect to each other. The company adds services in VPC-B (peered with VPC-A) but those services cannot resolve the Cloud Map DNS names. What is missing?

**Options:**
A) Cloud Map private DNS namespaces only work within a single VPC and cannot span VPC peering connections
B) The VPC-B DNS resolver needs to be configured with conditional forwarding rules to VPC-A
C) Cloud Map requires Route 53 Resolver inbound endpoints in VPC-A for cross-VPC resolution
D) VPC-B must be associated with the Cloud Map private DNS namespace for DNS resolution to work
...
### Question 14: SQS FIFO Queue Message Deduplication
**Scenario:** A financial system uses an SQS FIFO queue with content-based deduplication enabled. They send transaction records to the queue. During an incident, the same transaction is sent twice within a 1-minute window, but both messages are delivered to consumers, causing duplicate processing. What is the MOST likely cause?

**Options:**
A) Content-based deduplication only works for messages sent more than 5 minutes apart
B) The two transaction messages have different MessageAttributes, which are included in deduplication hash
C) Content-based deduplication uses the entire message body, and some timestamp or UUID field differs between messages
D) FIFO queues require explicit MessageDeduplicationId; content-based deduplication is only a fallback
...
### Question 15: Application Load Balancer with Lambda Targets Multi-Value Headers
**Scenario:** An ALB routes traffic to Lambda functions. The Lambda function receives HTTP requests with multiple Cookie headers (from different sources like browser, proxy). When the function accesses the headers, it only sees one Cookie value. What is the issue and solution?

**Options:**
A) ALB limits HTTP headers to single values; use NLB for multi-value header support
B) ALB combines duplicate headers into a comma-separated list; the Lambda must parse it
C) Lambda functions receive headers in a different format when invoked by ALB; check the multiValueHeaders field in the event
D) This is a limitation of Lambda function URLs; use API Gateway instead for full HTTP header support
